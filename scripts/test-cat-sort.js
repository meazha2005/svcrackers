...
async function testCatSort() {
  ...
  const [rows] = await connection.query(`
    SELECT id, name, (name + 0) as num_prefix
    FROM \`${prefix}categories\`
    ORDER BY (name + 0) ASC, name ASC
  `);

  console.log('Categories sorted naturally:');
  rows.forEach(r => {
    console.log(`ID: ${r.id} | NumPrefix: ${r.num_prefix} | Name: "${r.name}"`);
  });

  await connection.end();
}

testCatSort().catch(console.error);
